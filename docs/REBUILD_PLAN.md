...
- **Phase 2: in progress.**
  ...
  - Splash screen (2026-09-26): overlay 6 (F2, then information), from `assets/splash.petmate`. Verified in 40 and 80 columns on .23 and .237. VDC colour 12 (RGBI dark yellow) depends on the display's fix: brown on one, orange/olive on the other; accepted as is.
  - Compiler (2026-09-26): built with the official Oscar64 release v1.32.273 (main 546b627 broke the start-up console scrolling). With it verified on .23: start-up screen, REU detection, browser navigation, cursor left = directory up from the left column, v4 progress bar while reading, re-order, slot U.
