...
int main(int argc, char **argv)
{
    ...
    if (argc != 4 || sizeof(struct SlotStruct) != SLOTSIZE || sizeof(struct ConfigStruct) != 1041)
    {
    ...
    printf("version=%d timeon=%d utc=%d verbose=%d host=%s\n", config.version, config.timeon,
           config.secondsfromutc, config.verbose, config.host);
